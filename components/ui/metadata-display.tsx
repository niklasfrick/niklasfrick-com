...
export function MetadataDisplay({ className = '' }: { className?: string }) {
    ...
    return (
        <div className={className}>
            {/* Title */}
            <h1 className="text-2xl font-medium mb-4">{metadata.title}</h1>

            {/* Description */}
            {metadata.description && (
                <p className="text-lg text-zinc-600 dark:text-zinc-400 mb-6">{metadata.description}</p>
            )}
            ...
            <div className="flex items-center space-x-2 text-sm text-zinc-500 dark:text-zinc-400">
                ...
                {isUpdated && metadata.lastUpdated && (
                    <>
                        <span>•</span>
                        <span>Zuletzt aktualisiert {getRelativeTime(metadata.lastUpdated)}</span>
                    </>
                )}
            </div>
        </div>
